...
module Timer : sig
  ...
  type info = API.ref_task

  val create : info -> t

  val add_child : t -> child:t ref -> t

  val complete : t -> t

  val json_str : t ref -> string
end = struct
  ...
  type timer = Running of Mtime_clock.counter | Done of duration

  type info' = timer * info

  (* multiway tree structure *)
  type t = T of info' * t ref list

  let create info : t =
    let ctr = Mtime_clock.counter () in
    let info' = (Running ctr, info) in
    let children = [] in
    T (info', children)

  let add_child (T (info, children)) ~child =
    let children = child :: children in
    T (info, children)

  let duration ctr : duration =
    Mtime_clock.count ctr |> Mtime.Span.to_s |> Printf.sprintf "%fs"

  let complete = function
    | T ((Running ctr, task), children) ->
        T ((Done (duration ctr), task), children)
    | T ((Done _, _), _) as x ->
        x

  let status = function
    | Running ctr ->
        Printf.sprintf "Running %s" (duration ctr)
    | Done x ->
        Printf.sprintf "Done %s" x

  let rec json_str x =
    let (T ((timer, info), children)) = !x in
    Printf.sprintf {|{ "task_ref": "%s", "status": "%s", "subtasks": [ %s ] }|}
      (Ref.short_string_of info) (status timer)
      (List.map json_str children |> String.concat ", ")
end
...
let complete {timer} = timer := Timer.complete !timer

let debug_times {timer} = Timer.json_str timer
...
let get_initial () =
  let task_id = Ref.make_dummy "initial_task" in
  {
    session_id= None
  ; task_id
  ...
  ; dbg= "initial_task"
  ; client= None
  ; timer= ref (Timer.create task_id)
  ; test_rpc= None
...
let make_subcontext ~__context ?task_in_database task_name =
  ...
  __context.timer := Timer.add_child !(__context.timer) ~child:subcontext.timer ;
