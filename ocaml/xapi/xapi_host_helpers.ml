...
let xen_compatible () =
  try
    ...
    info "ROB: the Toolstack is compatible with the current Xen and libxenctrl versions";
    true
  with
  | Forkhelpers.Spawn_internal_error(_, _, Unix.WEXITED 2) ->
    ...
    warn "ROB: the current Xen version is incompatible with the Toolstack";
    false
  | Forkhelpers.Spawn_internal_error(_, _, Unix.WEXITED 127) ->
    ...
    warn "ROB: the current libxenctrl version is incompatible with the Toolstack";
    false
  | _ ->
    error "ROB: unexpected error when calling list_domains";
    false

let disable_of_xen_incompatible ~__context =
  if not (xen_compatible ()) then
...
let consider_enabling_host_nolock ~__context =
  ...
  if not !user_requested_host_disable && (not ha_enabled || all_pbds_ok) && xen_compatible () then begin
